...
- **Or add the gate yourself**, mirroring (not tightening) the checks the
  ...
  <!-- cost-guard-if:begin (generated by scripts/generate-cost-guard.mjs from src/authorization.ts and action.yml — do not edit by hand; run `npm run generate:cost-guard`) -->
  ```yaml
  jobs:
    issue-bot:
      if: >-
...
review comments and thread context, read the PR's own title/description/
changed files for supporting context, react to the triggering comment, and
reply inline in its review thread once an issue is filed — all of these are
write operations on the `pulls` resource, so `pull-requests: read` alone
gets a 403).
...
If you change `AUTHORIZED_ASSOCIATIONS` in `src/authorization.ts` or the
`mention` input's default in `action.yml`, run `npm run generate:cost-guard`
and commit the result. It regenerates the cost-guard `if:` embedded in
[reusable.yml](.github/workflows/reusable.yml) and the copy-pasteable
example in this README from those two files, so the three copies of that
